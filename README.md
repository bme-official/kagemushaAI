...
- `NEXT_PUBLIC_VRM_MODEL_URL`（公開可能なVRMファイルURL）
...
## VRoid / 音声チャット（実装済みMVP）

- 埋め込みチャット画面 `/embed/chat` に VRoid パネル領域を追加
- three.js + `@pixiv/three-vrm` で `NEXT_PUBLIC_VRM_MODEL_URL` のVRMを描画
- 音声入力（STT）: ブラウザ標準 `SpeechRecognition` を利用
- 音声読み上げ（TTS）: `speechSynthesis` で assistant 応答を読み上げ
- 設定ファイル:
  - `src/config/avatar.runtime.config.ts`
  - `src/config/voice.config.ts`

補足:

- VRoidは現時点でプレースホルダー実装です。`avatar.runtime.config.ts` の `modelUrl` を設定すると iframe で表示可能です。
- TTL/Realtime音声基盤は `voice.config.ts` の `transport` と `VoiceControls` の TODO を起点に差し替えできます。
- VRMファイルはGit管理せず、公開URL（WordPressメディア/Supabase Storage等）で指定してください。
